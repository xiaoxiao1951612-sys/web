...
export default function AdminEditExperience() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const selectedExperience = experiences[parseInt(id || '0')];
  ...
  const handleChange = (field: keyof ExperienceItem, value: string) => {
    setExperience(prev => prev ? ({ ...prev, [field]: value }) : null);
  };
  ...
  const handleSaveExperience = async () => {
    ...
    try {
      ...
      // 更新实习经历
      experiences[parseInt(id || '0')] = experience;
      
      // 使用数据服务保存
      const success = await saveExperiences(experiences);
      
      if (success) {
        toast.success('实习经历更新成功，所有访问者将看到最新内容');
        navigate('/admin/dashboard');
      } else {
      ...
      >
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center">
            ...
            <h1 className="ml-4 text-xl font-bold">编辑实习经历</h1>
          </div>
           <button
